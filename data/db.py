...
Connections are opened per-operation and alway"""
Persistent store for things the USER creates in the app — releases now, and
backlog + goals soon. Uses Neon Postgres when DATABASE_URL is set (survives
redeploys); falls back to local SQLite otherwise (fine for local dev).
...
def init(cfg):
    ...
    goals_pg = """CREATE TABLE IF NOT EXISTS goals (
        id SERIAL PRIMARY KEY, text TEXT, source TEXT DEFAULT 'manual',
        done BOOLEAN DEFAULT FALSE, done_at TIMESTAMPTZ,
        subs_at_done INTEGER, views_at_done INTEGER,
        created_at TIMESTAMPTZ DEFAULT now())"""
    goals_sql = """CREATE TABLE IF NOT EXISTS goals (
        id INTEGER PRIMARY KEY, text TEXT, source TEXT,
        done INTEGER DEFAULT 0, done_at TEXT,
        subs_at_done INTEGER, views_at_done INTEGER, created_at TEXT)"""
    try:
        with _conn(cfg) as c:
            ...
            cur.execute(goals_pg if pg_enabled(cfg) else goals_sql)
    except Exception:
...
# ---------- Goals (checkable, with growth snapshot at completion) ----------
def add_goal(cfg, text, source="manual"):
    """Add a goal if an identical open one doesn't already exist. Returns True if added."""
    existing = list_goals(cfg)
    if not existing.empty and (existing["text"] == text).any():
        return False
    ph = _ph(cfg)
    with _conn(cfg) as c:
        cur = c.cursor() if pg_enabled(cfg) else c
        if pg_enabled(cfg):
            cur.execute(f"INSERT INTO goals(text,source) VALUES({ph},{ph})", (text, source))
        else:
            cur.execute("INSERT INTO goals(text,source,done,created_at) VALUES(?,?,0,?)",
                        (text, source, datetime.now(timezone.utc).isoformat()))
    return True
...
def list_goals(cfg):
    try:
        with _conn(cfg) as c:
            return pd.read_sql_query("SELECT * FROM goals ORDER BY created_at DESC", c)
    except Exception:
        return pd.DataFrame()


def complete_goal(cfg, gid, subs, views):
    ph = _ph(cfg)
    now = datetime.now(timezone.utc).isoformat()
    with _conn(cfg) as c:
        cur = c.cursor() if pg_enabled(cfg) else c
        done_val = True if pg_enabled(cfg) else 1
        cur.execute(
            f"UPDATE goals SET done={ph}, done_at={ph}, subs_at_done={ph}, views_at_done={ph}"
            f" WHERE id={ph}", (done_val, now, int(subs), int(views), gid))


def reopen_goal(cfg, gid):
    ph = _ph(cfg)
    with _conn(cfg) as c:
        cur = c.cursor() if pg_enabled(cfg) else c
        done_val = False if pg_enabled(cfg) else 0
        cur.execute(f"UPDATE goals SET done={ph}, done_at=NULL, subs_at_done=NULL,"
                    f" views_at_done=NULL WHERE id={ph}", (done_val, gid))


def delete_goal(cfg, gid):
    ph = _ph(cfg)
    with _conn(cfg) as c:
        cur = c.cursor() if pg_enabled(cfg) else c
        cur.execute(f"DELETE FROM goals WHERE id={ph}", (gid,))
s closed, so Neon's free-tier
connection limit isn't exhausted.
"""
import sqlite3
import contextlib
